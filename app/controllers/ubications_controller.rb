class UbicationsController < ApplicationController
  ...
  def create
    ...
    @ubication.user_id = current_user.id 
    respond_to do |format|
      if @ubication.save
        format.html { redirect_to :controller => 'images', :action => "new", :id_pub => @ubication.publication_id }
  ...
  def update
    respond_to do |format|
      if @ubication.update(ubication_params)
        @image = Image.find_by(publication_id: @ubication.id)
        format.html {  redirect_to :controller => 'images', :action => "new", :id_pub => @ubication.publication_id}
  ...
  private
    ...
    def ubication_params
      params.require(:ubication).permit(:country, :city, :town, :street, :number_out, :number_in, :colony, :postal, :publication_id, :user_id)
    end
end
